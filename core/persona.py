...
def blog_block() -> str:
    ...
    default_tone = (
        ...
        "**否定的・反論的・批判的な表現は極力避け、建設的・肯定的に書く**"
        "（『〜は誤りです』『〜してはならない』『〜は意味がない』のような否定形は控え、"
        "『〜が有効です』『〜の方が成果が出ます』『〜がポイントです』のような肯定形で書く）。"
        "哲学的・抽象的・内省的な思索や、過度なエッセイ調・体験談ベースの展開には流れない。"
...
def posts_block() -> str:
    ...
    default_tone = (
        "**否定的・反論的・批判的な表現は極力避け、建設的・肯定的に書く**。"
        "『〜は誤り』『〜してはならない』『〜は無意味』のような否定形は控え、"
        "『〜が有効』『〜が勝つ』『〜の方が成果が出る』のような肯定形で書く。"
        "哲学的・内省的な思索ではなく、読者の実務に直接効くデータ・事例・How-toを主軸にする。"
    )
    tone_constraints = (p.get("tone_constraints") or "").strip() or default_tone
    return f"""\
## X投稿の人格設定（必ず守ること）
