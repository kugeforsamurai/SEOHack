...
## 記事・投稿で表現するときのガイド（重要）
- **HookHackの話（社名・サービス名・PoC案内・自社実践事例）は基本「まとめ章」のみに集約する**
- **通常のH2では HookHack を一切名指ししない**。一般論として「動画広告は視聴維持率というシグナルで媒体学習が深まる」「LPと素材から訴求案を発散すれば最短5分で複数パターンを試せる」のように、競合製品も含めた選択肢として書くスタイル
- **独立した「自社実践コーナー」H2/H3 はデフォルトで作らない**。HookHack の話はまとめ章の最後の段落で次の一歩として滲ませるだけ
...
- **まとめH2のタイトルは「まとめ」だけにしない**。記事の内容を反映した具体的なタイトルにする。ただし「まとめ」「総括」「結論」「次の打ち手」「次の一歩」のいずれかを含めて読者にラスト章だと分かるようにする
  例: 「視聴維持率改善の次の打ち手」「動画PoCを始めるためのまとめ」「AI×SEO実装の総括」「静止画→動画移行 — 結論」「クリエイティブ運用の次の一歩」
- **「### CTA」「### お問い合わせ」「### 誘導」のような独立セクションは絶対に作らない**。CTAはまとめH2 内の最後の段落として書く
...
### {{記事内容を反映した具体タイトル（「まとめ」「総括」「結論」「次の打ち手」「次の一歩」のいずれかを含めること）}}（H2 / 最終セクション）
- 推定字数: 400〜600字
- 内容メモ:
  - 核メッセージ再掲（2〜3行）
  - 次の打ち手3つ（箇条書き、表現を多様に：「最初に手をつけるポイント」「すぐ試せる3案」「導入の3ステップ」など）
  - **最後の段落で次の一歩を提示**：HookHack目的1（動画PoC）or 目的2（自社AI実践）への接続を、煽らず自然な「次の一歩」として滲ませる。**HookHackの話題はここでのみ触れる**（他のH2では一切触れない）。
    例: 「静止画運用が続いているなら、まず1パターンだけ動画を回してCPA差分を観測してみると判断材料が得られます」
- **「CTA」「誘導」「告知」のような事務的見出しを独立セクションにしない**。CTA要素はこのまとめH2内の最後の段落に組み込む
...
def section_prompt(
    ...
    if is_self_practice:
        ...
        role_hint = (
            "\n- このセクションは『まとめ + 次の一歩』。以下を1つの自然な流れで書く（独立セクションに分けない）："
            "\n  1. 記事の核メッセージ再掲（2〜3行）"
            "\n  2. 次の打ち手3つ（箇条書き、表現を多様に：「最初に手をつけるポイント」「すぐ試せる3案」「導入の3ステップ」など）"
            "\n  3. **最後の段落で次の一歩を自然に滲ませる**：HookHack 目的1（動画PoC）or 目的2（自社AI実践）への接続を、煽らず読者の自然な選択肢として提示する。"
            "「弊社サービスは…」「お問い合わせは…」のような事務的セールス文は禁止。"
            "例: 「静止画運用が続いているなら、まず1パターンだけ動画を回してCPA差分を観測してみると判断材料が得られます」"
            "\n- **本セクション内に『CTA』『お問い合わせ』『誘導』のような事務的見出しを作らない**"
        )
    elif is_cta:
        role_hint = (
            "\n- このセクションは記事末の『次の一歩』。HookHack 目的1（動画PoC）or 目的2（自社AI実践）に自然に着地、ハードセル禁止、1〜2段落"
            "\n- **見出しに『CTA』『誘導』『お問い合わせ』『コンタクト』のような事務的単語を使わない**。"
            "代わりに **内容に即した自然な見出し** にする（例: 「次の一歩」「実装に向けて」「動画PoCで成果差を確かめる」「より深く取り組みたい方へ」等）"
...
{current_block}"""
    ...
    # 通常H2では HookHack を名指ししない（まとめ集約ルール）
    # summary セクション(is_summary=True)でのみ HookHack 言及を許可
    hookhack_scope_hint = ""
    if not is_summary and not is_cta and not is_self_practice:
        hookhack_scope_hint = (
            "\n- **このセクションでは HookHack を一切名指ししない**。社名・サービス名・自社事例の言及は禁止。"
            "HookHack に関する話題はまとめH2のみで扱う（記事内で1箇所に集約する設計）"
        )
