...
## 記事・投稿で表現するときのガイド（重要）
- **HookHackの話（社名・サービス名・PoC案内・自社実践事例）は基本「CTA章（記事末の次の一歩章）」のみに集約する**
- **通常のH2・まとめH2では HookHack を一切名指ししない**。HookHack のセールスポイント（LP起点の訴求案発散、最短5分で動画パターン生成、Google広告連携の自動解析）も通常H2では具体的に書かない
- **独立した「自社実践コーナー」H2/H3 はデフォルトで作らない**。HookHack の話はCTA章で次の一歩として滲ませるだけ
- 「自社実践コーナーを入れて」「自社事例を厚く」等のユーザー指示がある場合のみ、独立コーナーを設置する
- 過度に煽らない。読者が自分で「だから動画PoC＝HookHackが合うかも」と気づく余白を残す
- **記事構成**: 通常H2群 → まとめ章（純粋集約） → CTA章（次の一歩、HookHack誘導）の3パートに分ける

## お題と HookHack 目的の整合性ルール（最重要・厳守）
- **お題が広告クリエイティブ / 広告運用 / 動画広告 / CPA・CVR最適化 などと直接関係しない場合**（例: SEOツール選び、CRM導入、組織マネジメント、採用、人事、IR、データ分析の一般論など）、**動画 vs 静止画の比較や CPA分解メカニズムを記事に持ち込まない**。
- 上記の場合、HookHack の戦略目的は **目的2側（自社AI実践 → SEO/SNS/LP/CRM等への応用）に寄せる**。CTA章での誘導も「動画PoC」ではなく「AI活用の支援」に着地させる。
- お題と動画広告が無理に結びついていないかを毎回チェック。比較・例示が記事の本筋から外れているなら、その例示は丸ごと省略する（無理に動画話を入れると読者が違和感を持つ）。
- 動画広告メカニズム（視聴維持率がシグナル → 媒体学習が深まる等）は、**お題が動画・広告クリエイティブと直結する時のみ** 持ち込む理論。SEO記事・組織論記事・AI活用記事の中に強引に入れない。
"""


def _disable_hookhack_block(disable_hookhack: bool = False) -> str:
    """記事から HookHack 言及を完全に外したい場合のオーバーライドブロック。
    True ならプロンプト末尾に最重要指示として注入される。"""
    if not disable_hookhack:
        return ""
    return """

## ⚠️ この記事では HookHack について一切触れない（最優先・上書き指示）
上記の HookHack 戦略・セールスポイント・着地点はすべて無視する。記事には以下を一切含めない：
- 「HookHack」「HookHack社」「HookHackでは」「弊社では」などの社名・自社言及
- 動画広告PoCサービス、AIによる訴求案発散・パターン生成、Google広告連携の自動解析など、HookHack製品の機能紹介
- 「自社実践コーナー」「自社事例」「私たちのチームでは」のような自社経験の差し込み
- まとめ章の最後で HookHack への誘導・問い合わせ・サービス案内に着地させる

記事は **中立的な情報源** として、業界一般のベストプラクティス・データ・他社事例のみで構成する。
まとめ章は「業界の方向性」「読者が次に試せる手」として書き、特定企業への誘導はしない。
"""
...
- **H2の論点と合致する事例があれば**、cases.csv から「誰が／何をして（施策）／どうなったか（結果_数字）／示唆」を1〜2件、具体名と具体数字で引用する。
  例: 「Nike が静止画→動画切替で CTR 2.1倍」のように、社名・施策・数字を含む形で
...
- **まとめH2（最終から2番目、300〜500字）= 純粋な集約**：核メッセージ再掲 + 記事全体の論点振り返り + 次の打ち手3つ。**事例は引用しない**（具体的な社名・数字は出さない）。**HookHack には触れない**。
  - タイトル例: 「視聴維持率改善のまとめ」「動画PoC論点の総括」「AI×SEO実装の結論」「静止画→動画移行 — ラップアップ」（「まとめ」「総括」「結論」「ラップアップ」のいずれかを含める）
- **CTA H2（最終、200〜400字）= 次の一歩への誘導**：HookHack 目的1（動画PoC）or 目的2（自社AI実践）への接続を、煽らず自然な「次の一歩」として滲ませる。**事例は引用しない**。
...
- 内容メモ:
  ...
  - **HookHackには触れない**（CTA章で触れる）

### {{記事内容を反映したCTAタイトル（「CTA」「誘導」「お問い合わせ」を使わず、内容を表す自然な見出しにする）}}（H2 / 最終）
- 推定字数: 200〜400字
- 内容メモ:
  - 読者が今すぐ取れるアクションを1〜2個（一般論で）
  - **最後の段落でHookHackへの次の一歩を自然に提示**：目的1（動画PoC）or 目的2（自社AI実践）への接続を、煽らず読者の選択肢として滲ませる
    例: 「静止画運用が続いているなら、まず1パターンだけ動画を回してCPA差分を観測してみると判断材料が得られます」
...
- **まとめH2（最終セクション、400〜600字）= 核メッセージ再掲 + 具体的な次の打ち手3つ + 最後の段落でHookHack目的1 or 目的2への次の一歩を自然に滲ませる**
- **まとめH2のタイトルは「まとめ」だけにせず、記事内容を反映した具体タイトルに**（「視聴維持率改善の次の打ち手」「動画PoCを始めるためのまとめ」等）。ただし「まとめ」「総括」「結論」「次の打ち手」「次の一歩」のいずれかを含めて読者にラスト章だと分かるようにする
- **「### CTA」「### お問い合わせ」のような独立セクションは絶対に作らない**。CTAはまとめH2内の最後の段落として書く
...
### まとめ（H2 / まとめ + 次の一歩 を統合した最終セクション）
- 推定字数: 400〜600字
- 内容メモ:
  - 核メッセージ再掲（2〜3行）
  - 次の打ち手3つ（箇条書き、表現を多様に：「最初に手をつけるポイント」「すぐ試せる3案」「導入の3ステップ」など）
  - 最後の段落で **HookHack目的1 or 目的2 への次の一歩を自然に滲ませる**（独立した「CTA」セクションは作らない）
```
...
def section_prompt(
    ...
    if is_self_practice:
        ...
        role_hint = (
            ...
            "\n- **HookHack には触れない**（CTA章で触れる）"
            "\n- **本セクション内に『CTA』『お問い合わせ』『誘導』のような事務的見出しを作らない**"
        )
    elif is_cta:
        role_hint = (
            "\n- このセクションは記事末の『次の一歩』章。HookHack 目的1（動画PoC）or 目的2（自社AI実践）に自然に着地、ハードセル禁止、200〜400字"
            "\n- 構成: (1) 読者が今すぐ取れるアクションを1〜2個一般論で → (2) 最後の段落で HookHack への次の一歩を自然な選択肢として滲ませる"
            "\n- **見出しに『CTA』『誘導』『お問い合わせ』『コンタクト』のような事務的単語を使わない**。"
            "代わりに **内容に即した自然な見出し** にする（例: 「次の一歩へ」「最初の一歩」「動画PoCで成果差を確かめる」「実装に向けて」「踏み出すための準備」「より深く取り組みたい方へ」「アクションプラン」等）"
...
{current_block}"""
    ...
    # HookHack 言及が許されるのは CTA章 と 自社実践コーナー のみ。
    # 通常H2・まとめH2では HookHack を一切名指ししない（CTA集約ルール）
    hookhack_scope_hint = ""
    if not is_cta and not is_self_practice:
        hookhack_scope_hint = (
            "\n- **このセクションでは HookHack を一切名指ししない**。社名・サービス名・自社事例の言及は禁止。"
            "HookHack に関する話題は CTA章でのみ扱う（記事内で1箇所に集約する設計）"
        )
...
{persona.blog_block()}
{_topic_context_block(angle_hint, interests_hint, user_direction)}{revision_block}{_disable_hookhack_block(disable_hookhack)}
## タスク
...
- **事例企業の業界を一言で添える（必須）**: 「〜社」「Nike」「Sephora」などの社名を出す際、読者が瞬時にどんな会社か分かる業界・業態のミニ説明を社名と一緒に書く。
  例: 「スポーツ用品大手の Nike は…」「化粧品EC の Sephora は…」「米SaaS の HubSpot は…」「日本の食品メーカー◯◯社は…」
