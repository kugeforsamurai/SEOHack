...
- このお題に関して、現場でよく見るパターン（やり方／状況／アプローチ）を **2〜3 個** 軽く並べる
- パターン名は短く、読者が「あ、これ自分のところもある」と感じるレベルの粒度
- 例（あくまで構成イメージ）:
  - 「LP制作には大きく分けて〇〇型、〇〇型、〇〇型の3つのアプローチがある」
  - 「動画広告のクリエイティブPDCAは、内製型・外注型・ハイブリッド型に分かれる」
...
- 抽象論を読者の状況に **戻す**。「であれば、あなたが今〇〇という状況なら、まずどこを見るべきか」と問いかける
...
- 数字 or 固有名詞を最低1つ入れて具体性を出す。ただし cases.csv の社名・数字に紐づく場合のみ
- 煽らない（「衝撃！」「絶対！」「必読！」「決定版！」等は禁止）
- 結論を最初に言い切らない（記事を読む動機を残す）
- 読者が **記事冒頭で「これは自分のための記事だ」と気づく** ことを最優先
- 第1段の「パターン」は本文（H2群）と重複させない。本文を読む前にパターンを並べることで、本文の章ごとに「あ、これさっき言ってた〇〇型の話か」と接続させるのが狙い
