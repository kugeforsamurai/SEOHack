            ...
            for idx, sec in enumerate(sections):
                ...
                with st.container(border=True):
                    ...
                    if _has_number and _case_names_in_memo:
                        ...
                        col_regen_memo, col_add_cases = st.columns(2)
                        with col_regen_memo:
                            if st.button(
                                "🔁 このH2のメモを再生成",
                                key=f"regen_memo_{idx}",
                                help="現在のメモを土台に、cases.csv 整合の新メモへ作り直す（社名が一致するものに置換 or 理論深掘りへ）",
                                width="stretch",
                            ):
                                with st.spinner("Gemini がメモを作り直しています..."):
                                    try:
                                        _cases_csv = cases_df.to_csv(index=False) if not cases_df.empty else ""
                                        _other_secs = [
                                    ...
                                    with st.spinner("ステップ1/2: Gemini が追加事例をリサーチしています..."):
                                        ...
                                        _new_cases = gemini_client.generate_json(
                                            prompts.cases_supplement_prompt(
                                                ...
                                                h2_title=sec_title,
                                                h2_memo=sec_memo,
                                                existing_cases_csv=_existing_csv,
                                                n_cases=4,
                                                angle_hint=angle_hint, interests_hint=interests_hint,
                                                user_direction=user_direction,
                                            )
...
# ============================================================
    ...
    if not outline:
        ...
        col_gen, col_info = st.columns([1, 2])
        with col_gen:
            if st.button("Geminiで重点セクションを提案", type="primary", width="stretch", help="画像案は⑤本文書き上がり後に本文ベースで別途提案する設計"):
    ...
    if not outline:
        ...
        with col_l:
            ...
            lead_memo = st.text_area(
                "📝 リード用メモ（任意 / ここに書いた指示は『✨ リード文を生成』時に最優先で反映される）",
                value=saved.get("lead_memo", ""),
                height=110,
                key="blog_lead_memo",
                placeholder=(
                    "論点指定の例: 「『内製 vs 外注 vs ハイブリッド』の3パターンで整理して」"
                    "／「PDCAサイクルの速さを成功要因の中心に据えて」\n"
                    "構成変更の例: 「200字程度の1段で簡潔に」"
                    "／「ストーリー形式で始めて」（→3段テンプレから逸脱可）"
                ),
                help=(
                    "ここに書いたメモはリード生成時にプロンプトに注入され、デフォルトの3段構成"
                    "（パターン→成功要因→具体化）より優先される。論点指定なら3段に組み込み、"
                    "構成・字数・トーン指定ならテンプレから逸脱可。sections.json に永続化される。"
                ),
            )
        ...
        if blog_md:
            ...
            # ---- 📷 画像案を本文から提案（初回 + 再考、blog_md完成後のみ表示） ----
            st.divider()
            _existing_imgs = review.get("images", [])
            _is_initial = not _existing_imgs
            if _is_initial:
                st.subheader("📷 画像案を本文から提案（初回）")
                st.caption(
                    "結合後の本文を踏まえて、Gemini が画像案を初回提案します。"
                    "1記事 3枚程度、種類を多様に（checklist / comparison_table / process_flow / data_chart）。"
                    "下の指示欄は空でもOK（空ならAIに完全に任せる）。"
                )
            else:
                st.subheader(f"📷 画像案を再考（現在{len(_existing_imgs)}件）")
                st.caption(
                    "本文を踏まえて画像案リスト全体を作り直し。"
                    "1記事 3枚程度、種類を多様に。重点セクションは触らず画像案だけ置き換わります。"
                )

            img_refine_feedback = st.text_area(
                "再考指示（初回は空でもOK / 入れるとその方向に作る）",
                placeholder=(
            ...
            _btn_label = (
                "📷 本文から画像案を初回提案" if _is_initial else "🔁 この指示で画像案を作り直す"
            )
            if st.button(
                _btn_label,
                ...
                with st.spinner("Geminiが画像案を生成中…（30〜60秒）"):
                    try:
                        import json as _json_ir
                        _current_imgs_json = _json_ir.dumps(
                            _existing_imgs, ensure_ascii=False, indent=2
                        )
                        # 初回かつ指示空なら「本文を踏まえて多様な画像案を提案して」とフォールバック
                        _feedback = img_refine_feedback.strip() or (
                            "初回提案。本文の内容を踏まえて、画像案を多様な種類で3枚程度提案してください。"
                            if _is_initial else "リスト全体を再考してください。"
                        )
                        new_images = gemini_client.generate_json(
                            prompts.image_refine_prompt(
                                topic, outline, blog_md,
                                _current_imgs_json, _feedback,
                                angle_hint=angle_hint,
                        ...
                        if not isinstance(new_images, list):
                            ...
                            for k in list(st.session_state.keys()):
                                if k.startswith(("cl_", "tb_", "img_prompt_", "img_size_", "img_qual_")):
                                    del st.session_state[k]
                            st.success(f"画像案を {len(new_images)} 件で生成しました。⑤上部の「📷 画像（編集 + 生成）」で画像を作り直してください。")
                            st.rerun()
                    except Exception as e:
                        st.error(f"エラー: {e}")
